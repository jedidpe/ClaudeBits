...
description: "Build applications backed by Aurora PostgreSQL by leveraging this skill. It bundles direct database connectivity through the Aurora PostgreSQL MCP server for data plane operations (queries, table creation, schema management), and control plane operations (cluster creation), The steering file helps with Aurora PostgreSQL specific best practices. When developers work on database tasks, the skill dynamically loads relevant guidance - whether creating new Aurora clusters, designing schemas, or optimizing queries - so Kiro agent receives only the context needed for the specific task at hand."
---

# Aurora Postgres skill

## Overview

Build database-backed applications with Aurora PostgreSQL through seamless MCP server integration. This skill provides:
...
This skill combines comprehensive guidance for database design, query optimization, schema management, and operational excellence with direct MCP integration for both provisioned instances and Aurora Serverless v2

## Available Steering Files

This skill includes two comprehensive steering files that provide detailed guidance:
...
This skill uses the **awslabs.postgres-mcp-server** MCP server to provide direct integration with Aurora PostgreSQL clusters.
...
The skill uses the Aurora Postgres MCP server with the following configuration:
